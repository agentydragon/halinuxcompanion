...
class NotificationHints(BaseModel):
    ...
    urgency: UrgencyLevel | None = Field(default=None, description="Urgency level (0=low, 1=normal, 2=critical)")

    # Not yet implemented but defined in spec
    action_icons: bool | None = Field(default=None, description="Use icon names in actions instead of text")
    category: str | None = Field(
        default=None,
        description="Notification category (e.g., 'email.arrived', 'im.received')",
    )
    desktop_entry: str | None = Field(default=None, description="Desktop filename without .desktop suffix")
    image_path: str | None = Field(default=None, description="Alternative image location")
    ...
    resident: bool | None = Field(default=None, description="Keep notification after action invoked")
    sound_file: str | None = Field(default=None, description="Sound file path to play")
    sound_name: str | None = Field(
        default=None,
        description="Sound theme name from freedesktop.org sound naming spec",
    )
    suppress_sound: bool | None = Field(default=None, description="Suppress sound playback")
    transient: bool | None = Field(default=None, description="Hint for transient notifications")
    x: int | None = Field(default=None, description="X position hint")
    y: int | None = Field(default=None, description="Y position hint")
...
class DBusNotification(BaseModel):
    ...
    app_name: str = Field(description="The optional name of the application sending the notification. Can be blank.")
    ...
    app_icon: str = Field(description="The optional program icon. Can be empty string for no icon.")
    summary: str = Field(description="The summary text briefly describing the notification.")
    body: str = Field(default="", description="The optional detailed body text. Can be empty.")
    ...
    hints: NotificationHints = Field(default_factory=NotificationHints, description="Structured notification hints")
